...
def programsensor(devid, newdevid, interval):
    ...
    if rf4 == 0:
        ...
        programsensorR2(devid, newdevid, interval, rf4, 5)
    else:
        print("RF4 protocol detected")
        programsensorR4(devid, newdevid, interval, rf4, 5)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Configure RF Light Sensor")
        helpmessage("rflight.py")
    if len(sys.argv) < 4:
        interval = 5
    else:
    ...
    print("Configuring Light sensor with ID >>" + str(sys.argv[1]) +
